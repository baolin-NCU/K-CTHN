...
classes[7] = '64QAM'
#AP数据提取，作为人工特征
filename = f"{root_dir}/data/A_P_data_b.pickle"
...
def plot_confusion_matrix(cm, title='', cmap=plt.cm.Blues, labels=[]):
    ...
    plt.xlabel('Classification label', **axis_font)
    plt.grid(True)
#   批处理对测试集进行预测
...
for snr in snrs:
    ...
    plt.savefig(f"{save_dir}/comf_Matrix_for_snr=" + str(snr)+".png", format='png', dpi=600, bbox_inches='tight')  # 设置 dpi 参数以调整保存的图像质量
    #   拿到原始混淆矩阵对角线的元素并求和
